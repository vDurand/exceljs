...
class VmlAnchorXform extends BaseXform {
  ...
  getDefaultRect(model) {
    const l = model.refAddress.col;
    const lf = 6;
    const t = Math.max(model.refAddress.row - 2, 0);
    const tf = 14;
    const r = l + model.note.width;
    const rf = 2;
    const b = t + model.note.height;
    const bf = 16;
    return [l, lf, t, tf, r, rf, b, bf];
  }

  render(xmlStream, model) {
    const rect = model.anchor ? this.getAnchorRect(model.anchor) : this.getDefaultRect(model);
